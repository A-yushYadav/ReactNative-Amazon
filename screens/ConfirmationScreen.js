...
const ConfirmationScreen = () => {
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      const response = await axios.post("http://192.168.1.13:8000/orders", orderData)
  ...
  const pay = async () => {
    try {
      const options={
        ...
        currency :"INR",
        name : "Amazon",
        key :"rzp_test_THR8VVcPCi0GVG",
        amount :total * 100,
        prefill:{
          email:"ayush020yv@gmail.com",
          contact :"7999830695",
  ...
  return (
    <ScrollView style={{ marginTop: 55 }}>
            ...
            {addresses?.map((item, index) => (
              <Pressable style={{ borderWidth: 1, borderColor: "#D0D0D0", padding: 10, flexDirection: "row", alignItems: "center", gap: 5, paddingBottom: 17, marginVertical: 7, borderRadius: 6 }}>
                {selectedAddress && selectedAddress._id === item?._id ? (
                  ...
                  <Entypo onPress={() => setSelectedAddress(item)} name="circle" size={24} color="grey" />
                )}
      ...
      {currentStep == 1 && (
        <View style={{ marginHorizontal: 20 }}>
          <Text style={{ fontSize: 17, fontWeight: "bold" }}>Choose your Deliver Options</Text>
          <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: "white", padding: 8, gap: 7, borderColor: "#D0D0D0", borderWidth: 1, marginTop: 10 }} >
            {option ? (
              ...
              <Entypo onPress={() => setOption(!option)} name="circle" size={24} color="grey" />
            )}
            <Text style={{ flex: 1 }}>
      ...
      {currentStep == 2 && (
        <View style={{ marginHorizontal: 20 }}>
          <Text style={{ fontSize: 17, fontWeight: "bold" }}>Select Payment Method</Text>
          <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: "white", padding: 8, gap: 7, borderColor: "#D0D0D0", borderWidth: 1, marginTop: 10 }} >
            {selectedOption === "cash" ? (
              <FontAwesome5 name="dot-circle" size={24} color="#008397" />
            ) : (
      ...
      {currentStep == 3 && selectedOption === "cash" && (
        <View style={{ marginHorizontal: 20 }}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", backgroundColor: "white", padding: 8, gap: 7, borderColor: "#D0D0D0", borderWidth: 1, marginTop: 10, justifyContent: "space-between" }} >
            <View>
          ...
          <View style={{ backgroundColor: "white", padding: 8, borderColor: "#D0D0D0", borderWidth: 1, marginTop: 10, }}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
              <Text style={{ fontSize: 15, color: "grey", fontWeight: "500" }}>Items</Text>
              <Text style={{ fontSize: 15, color: "grey" }}>₹{total}</Text>
            </View>

            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
              <Text style={{ fontSize: 15, color: "grey", fontWeight: "500" }}>Delivery</Text>
              <Text style={{ fontSize: 15, color: "grey" }}>₹0</Text>
            </View>

            <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 }}>
              <Text style={{ fontSize: 20, fontWeight: "bold" }}>Order Total</Text>
              <Text style={{ fontSize: 17, color: "#C60C30", fontWeight: "bold" }}>₹{total}</Text>
            </View>
          </View>
          <View style={{ backgroundColor: "white", padding: 8, borderColor: "#D0D0D0", marginTop: 10, borderWidth: 1 }}>
          ...
          <Pressable onPress={handlePlaceOrder} style={{ backgroundColor: "#FFC72C", padding: 10, borderRadius: 20, justifyContent: "center", alignItems: "center", marginTop: 15 }}>
